...
def download_file(loc, bucketname, s3, file):
    os.chdir(loc)
    if os.path.isdir(file):
        print(
    
    try:
        s3.Bucket(bucketname).download_file(file, file)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == "404":
            print("The object does not exist.")
        else:
            raise
    print("Downloadload complete for location %s at %s" % (loc, str(datetime.now())))
...
if __name__=="__main__":
    ...
    parser.add_argument('-f', '--file', required=True, help="Files to download")
    args = parser.parse_args()

    s3=boto3.client('s3')
    download_file(args.data_source, args.bucket_name, s3, args.file)
